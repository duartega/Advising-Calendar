...
select * from users;
  ...
  UPDATE AdvisingTimes set Student_ID_Registered = NULL where uniId = 128;
...
delimiter ;
  ...
  Delimiter //
  create procedure getProfessorNameByStudentId(_student_id int)
  begin
		-- Get the students current advisor name
		select u.fname, u.lname from users as u
		join students as s on s.instructor_id = u.id
...
select * from AdvisingTimes where Student_ID_Registered = 5;

select * from users; -- has ID
select * from AdvisingTimes; -- has instructor id as id

select * from students; -- has instructor_id and user_id 



select a.uniId, a.Day, a.StartTime, a.EndTime, a.TimeBlock
