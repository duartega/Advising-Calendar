...
class EnhancedTable extends React.Component {
  ...
  update = (props) => {
    ...
    axios.get(`/Student/getAdvisingTimes/${instructor_id}`).then(result => {
        console.log("MADE IT HERE AT LEAST")
  ...
  async componentDidMount() {
    const { id, instructor_id } = this.props;
    let array = [];
    console.log("MADE IT HERE AT LEAST IN COMPONENT DID MOUNT... student_id = ", id)
    console.log("MADE IT HERE AT LEAST IN COMPONENT DID MOUNT... instructor_id = ", instructor_id)
    axios.get(`/Student/getAdvisingTimes/${instructor_id}`).then(result => {
      for(let i = 0; i < result.data.length; i++) {
        // console.log(result.data[i]['Day'])
        // console.log(result.data[i]['StartTime'])
